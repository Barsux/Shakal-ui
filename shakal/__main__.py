...
class Grid:
    ...
    def button_pressed(self, idx):
        ...
        if self.ui.convertButton.styleSheet() == Styles.default_finished_button_style:
            self.ui.convertButton.setStyleSheet(Styles.default_button_style)
        self.ui.image_selected(self.barcode_selected, self.pages)
    ...
    def fill_grid_pushed(self):
        if not self.grid_filled:
            ...
            if self.barcode_selected == 0:
                self.ui.convertButton.setEnabled(False)
        if self.ui.convertButton.styleSheet() == self.ui.convertButtonFinishedStylesheet:
            self.ui.convertButton.setStyleSheet(Styles.default_button_style)
...
class PdfWorker(QThread):
    ...
    def run(self):
        ...
        images = tuple(
            sorted(
                map(lambda image: os.path.join(self.dir, image), images),
                key=lambda file: int(file.split('_')[-2]))
        )
        image_sample = Image.open(images[0])
        aspect_ratio = image_sample.height / image_sample.width
...
class Ui_MainWindow(QMainWindow):
    ...
    def setupUi(self):
        ...
        self.convertButton.setStyleSheet(Styles.default_button_style)
    ...
    def connectUi(self):
        ...
        self.fileDependance[1] += [self.prevBageButton, self.nextPageButton, self.fillPages, self.convertButton,
                                   self.clearButton]
    ...
    def convert(self):
        ...
        self.pdfworker = PdfWorker(self.tempPdfDir.name, self.numDocuments, self.tempImagesDir.name,
                                   self.grid.get_coordinates(), barcode_width=self.barcode_width)
